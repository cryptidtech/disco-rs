...
mod keys {
    ...
    #[test]
    fn diffie_hellman() {
        let xeddsa = DiscoXeddsa {};

        // Initiator generates ephemeral key and test static key
        let (i_e_pub, i_e_sec) = xeddsa.generate(&xeddsa, get_rng());
        let (i_s_pub, i_s_sec) = xeddsa.generate(&xeddsa, get_rng());

        // Responder generates ephemeral key and test static key
        let (r_e_pub, r_e_sec) = xeddsa.generate(&xeddsa, get_rng());
        let (r_s_pub, r_s_sec) = xeddsa.generate(&xeddsa, get_rng());

        // Responder does the first DH using initiator ephemeral public key
        // and responder ephemeral secret key
        let r_first_dh = xeddsa.get_shared_secret(&r_e_sec, &i_e_pub).unwrap();

        // Initiator does the first DH using the responder ephemeral public key
        // and initiator ephemeral secret key
        let i_first_dh = xeddsa.get_shared_secret(&i_e_sec, &r_e_pub).unwrap();
        ...
        let r_second_dh = xeddsa.get_shared_secret(&r_e_sec, &i_s_pub).unwrap();

        // Initiator does the second DH using the responder static public key
        // and the initiator ephemeral secret key
        let i_second_dh = xeddsa.get_shared_secret(&i_s_sec, &r_e_pub).unwrap();
        ...
        let r_third_dh = xeddsa.get_shared_secret(&r_s_sec, &i_e_pub).unwrap();

        // Initiator does the third DH using the responder ephemeral public key
        // and the initiator static secret key
        let i_third_dh = xeddsa.get_shared_secret(&i_e_sec, &r_s_pub).unwrap();
